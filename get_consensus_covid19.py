...
def main():
    ...
    parser.add_option("-d", "--d", dest="directory", help="directory with freqs files, files containing mutation frequencies from AccuNGS pipeline")
    ...
    for f in tqdm.tqdm(freqs_files):
        ...
        df_snps = df[(df["Rank"] == 0) & (df["Ref"] != "-") & (df["Pos"] >= 55) & (df["Pos"] <= 29836) & (df["Base"] != '-')]
        df_deletions = deletion_group_filter(df, initial_threshold=0.75*0.8, final_threshold=0.8)
        df = estimate_insertion_freq(df)
        df_insertions =insertion_group_filter(df, initial_threshold=0.75*0.8, final_threshold=0.8)
            ...
            if ref_base != '-': # not insertion
                ...
                if base == ref_base:
                    ...
                    if frequency < 0.8 and base != '-':
                        con += "N"
                        mutations_turned_ns = mutations_turned_ns.append({"Pos":new_loc, "Ref":ref_base, "Base":base, "Read_count":coverage, "Freq":frequency, "sample":sample}, ignore_index=True)
                    else: # freq >= 0.8 or deletion that already underwent group filtering
                        con += base
                        mutations = mutations.append({"Pos":new_loc, "Ref":ref_base, "Base":base, "Read_count":coverage, "Freq":frequency, "sample":sample}, ignore_index=True)
                loc = new_loc
                
            if ref_base == '-' and new_loc - loc < 1: # insertion 
                con += base
                mutations = mutations.append({"Pos":row["Pos"], "Ref":ref_base, "Base":base, "Read_count":coverage, "Freq":frequency, "sample":sample}, ignore_index=True)
